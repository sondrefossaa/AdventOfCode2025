...
int getFewestPresses(machine *curr_machine) {
  size_t len = strlen(curr_machine->switches);
  char *currSwitches = malloc(len + 1);
  if (currSwitches) {
    memset(currSwitches, '.', len);
    currSwitches[len] = '\0'; // Correct: len, not len+1
  }
  int ans = 0;
  printf("Processing %s \n", curr_machine->switches);
...
int main(void) {
  ...
  while (fgets(buffer, sizeof(buffer), input) != NULL) {
    ...
    machines[data_size]->buttons =
        (char **)malloc(sizeof(char *) * MAX_BUTTONS);
    ...
    while (token != NULL) {
      ...
      case '[': {
        ...
        if (len > 2 && token[len - 1] == ']') {
          ...
          machines[data_size]->switches =
              malloc(len - 1); // -2 for brackets, +1 for null term
      ...
      case '(': {
        if (machines[data_size]->button_count < MAX_BUTTONS) {
          machines[data_size]->buttons[machines[data_size]->button_count] =
              strdup(token);
      ...
      case '{': {
        ...
        while (*jolt_str != '}' && *jolt_str != '\0') {
          ...
          if (machines[data_size]->joltage_count < MAX_JOLTAGE) {
            machines[data_size]->joltage[machines[data_size]->joltage_count] =
                (int)value;
